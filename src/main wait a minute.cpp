...
public:
    ...
    void saveDataToFile2() {
        std::ofstream outputFile("Save.txt", std::ios::app);
    ...
    void initializePokemonIndices()
            {
                ...
                for (int i = 0; i < 6; ++i) {
                    ...
                    do {
                        ...
                        isFound = 0;
                        std::cout << "Used Pokemon Size: " << UsedPokemonSize << std::endl;
                        for (int j = 0; j < UsedPokemonSize; j++) {
                            std::cout << listOfPokemon[UsedPokemonIndices[j]].Name << " compared with " << listOfPokemon[randomPokemon].Name << std::endl;
                            if (UsedPokemonIndices[j] == randomPokemon) {
                                isFound = 1;
                                break;
                            }
                    ...
                    std::cout << listOfPokemon[PokemonChoicesIndices[i]].Name << std::endl;
                }
...
{
public:
     Game(sf::RenderWindow& window, std::string playerName, int gender)
        : window(window), playerName(playerName), gender(gender)
    {
...
{
public:
     HallOfFame(sf::RenderWindow& window, sf::Music& music)
    ...
    int readFromHOFFile() {
            ...
            int numberOfPlayers = currentLine / 5;
            return 0;
        }
    ...
    void handleInput(sf::Event event)
    {
        ...
        else if (event.type == sf::Event::MouseButtonPressed)
        {
            if (event.mouseButton.button == sf::Mouse::Left)
            {
                if (next.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y))
                {
                    index++;
                }
            ...
            if (event.mouseButton.button == sf::Mouse::Left)
            {
              if (NewGame.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y))
              ...
              else if (HallofFame.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y))
                {
                    music.stop();
                    window.setActive(false);
                    HallOfFame hallOfFame(window, music);
                    hallOfFame.display();
                    window.setActive(true);
                }
            }
...
class maingame {
    ...
    public:
        maingame(sf::RenderWindow& window, sf::Music& music) :
            ...
            characterID(window, playerName, gender),
